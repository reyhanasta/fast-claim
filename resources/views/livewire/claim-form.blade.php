    ...
    <div class="max-w-4xl mx-auto">
                                    ...
                                    <div class="space-y-3">
                                     ...
                                     @foreach ([5, 6, 7, 8] as $labNumber)
                                         <div class="space-y-3">
                                             ...
                                             <div wire:loading wire:target="labResultFile{{ $labNumber }}" class="flex items-center gap-2 text-emerald-600"><div class="animate-spin rounded-full h-4 w-4 border-2 border-emerald-600 border-t-transparent"></div><span class="text-xs font-medium">Uploading...</span></div>
                                             ...
                                             @if ($this->{'labResultFile' . $labNumber})<div class="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800"><flux:icon.check-circle class="w-5 h-5 text-emerald-600 dark:text-emerald-400 flex-shrink-0" /><flux:text size="sm" class="text-emerald-900 dark:text-emerald-100 truncate flex-grow">{{ is_object($this->{'labResultFile' . $labNumber}) ? $this->{'labResultFile' . $labNumber}->getClientOriginalName() : 'Hasil Lab ' . $labNumber }}</flux:text></div>@endif
                                         </div>
                                     @endforeach                                </div>
                            </div>
        ...
        @if(!$showUploadedData)
            <div
                ...
                <div class="p-8">
                    <div
                        class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center hover:border-emerald-500 dark:hover:border-emerald-400 transition-all duration-300 bg-gradient-to-br from-gray-50 to-emerald-50/30 dark:from-gray-900/50 dark:to-emerald-900/10">
                        <div class="space-y-2">
                            <div
                                class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-emerald-100 dark:bg-emerald-900/30 mb-4 mx-auto">
                                <flux:icon.document-plus class="w-8 h-8 text-emerald-600 dark:text-emerald-400" />
                            </div>
                            <div>
