...
class ClaimFormManual extends Component
{
    ...
    #[Validate('required|string|in:RJ,RI', message: 'Jenis rawatan harus RJ atau RI')]
    ...
    {
        // Update label based on jenis rawatan
        $this->sep_date_label = $this->jenis_rawatan === 'RI' ? 'Tanggal Pulang' : 'Tanggal SEP';
    }
    ...
    public function submit(
        ...
        try {
            ...
            $this->redirect(route('claim-form-manual'), navigate: true);
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    {
        return view('livewire.claim-form-manual');
    }
    ...
    /**
     * Process SEP file: validate and store for manual entry.
     * NOTE: This is for manual claim entry - NO data extraction.
     *
     * @throws \RuntimeException if SEP is invalid
     */
    private function processSepFile(): void
    {
        ...
        // Validate SEP PDF is readable and single page
        $pdfReadService->ensureSinglePage($this->sepFile);

        // NOTE: Skip data extraction - user will fill form manually
        // Do NOT call extractPdf() or extractPdfAssist()

        // Store file temporarily for later merging
